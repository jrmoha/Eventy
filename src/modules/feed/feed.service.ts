...
const recommendations_events = async (user_id: number, token: string) => {
...
export const get_home_events = async (req: Request): Promise<Event[]> => {
  const recommendations = await recommendations_events(
...
export const random_events = async (): Promise<Event[]> => {
  return Event.findAll({
    include,
    attributes,
    where: {
